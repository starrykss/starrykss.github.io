export const translations = {
  ko: {
    metaDescription:
      '웹과 모바일 서비스를 개발하는 김상순의 포트폴리오입니다.',
    brand: 'STARRYKSS',
    nav: [
      'About',
      'Experience',
      'Tech Stack',
      'Certificate',
      'Completion',
      'Contact',
    ],
    heroText: '안녕하세요!\n소프트웨어 개발자\n김상순입니다',
    heroActions: ['소개', '포트폴리오', '연락처'],
    quickFacts: [
      'React / Next.js 기반 프론트엔드 개발',
      'TypeScript, Tailwind CSS, Python, Azure, Azure AI Foundry, Codex 경험',
      '웹과 모바일에서 쓸 수 있는 앱 개발',
    ],
    aboutKicker: 'Profile',
    aboutTitle: 'About Me',
    githubProfileLabel: 'starrykss의 GitHub 프로필 보기',
    githubChartLabel: '최근 1년간 GitHub 기여 내역. 가로로 스크롤할 수 있습니다.',
    githubLoading: 'GitHub 활동을 불러오는 중입니다.',
    githubError: '활동을 불러오지 못했습니다. 위 GitHub 프로필에서 확인해 주세요.',
    githubSummary: '최근 1년간 {count}회 기여',
    githubDay: '{date}: 기여 {count}회',
    githubLess: '적음',
    githubMore: '많음',
    profileRole: 'Software Engineer',
    profileName: '김상순 · @starrykss',
    profileDescription:
      '프론트엔드뿐만 아니라 백엔드와 DevOps에도 관심이 많습니다. 새로운 기술을 배우고 서비스에 적용하는 걸 좋아합니다. 웹과 모바일에서 쓰기 편한 앱을 만들고, 더 빠르고 안정적으로 동작하도록 개선하고 있습니다.',
    overviewKicker: 'Overview',
    overviewTitle: 'Current Focus',
    overviewItems: [
      [
        'Primary',
        '웹과 모바일 환경에 맞는 화면을 설계하고 개발합니다.',
      ],
      [
        'Tooling',
        'React, Next.js, React Native, Flutter, TypeScript, Tailwind CSS, Azure AI Foundry, Codex',
      ],
      [
        'Expansion',
        'React Native와 Flutter로 앱을 만들며 AI 개발 경험도 쌓고 있습니다.',
      ],
    ],
    experienceKicker: 'Timeline',
    experienceTitle: 'Experience & Education',
    experienceSummary: '',
    experienceGroupTitle: '경력',
    educationGroupTitle: '학력',
    timeline: [
      [
        '2025.02 - Present',
        'PLAYBUNIZ Inc.',
        '풀스택 개발, CTO',
        'React, TailwindCSS, Next.js, TypeScript, Azure, AWS, GCP, React Native, Flutter, MongoDB, PostgreSQL, Figma, Linux',
      ],
      [
        '2024.10 - 2024.10',
        'APST Corporation',
        '풀스택 개발 인턴',
        'React, FastAPI, Python, Redis, PostgreSQL',
      ],
      [
        '2024.02 - 2024.04',
        'Torooc Corporation',
        '프론트엔드 개발 인턴',
        'React, Styled Components, MobX, AWS, Python, ROS2',
      ],
      [
        '2023.04 - 2023.09',
        'Microsoft Korea AI School',
        '교육생',
        'HTML/CSS/JavaScript, Python, PyTorch, OpenCV, Flask, PyQt, Azure, Git',
      ],
      [
        '2016.03 - 2023.08',
        'Catholic University of Korea',
        '학사',
        '컴퓨터정보공학 전공, 미디어공학 복수전공',
      ],
    ],
    stackKicker: 'Capabilities',
    stackTitle: 'Tech Stack',
    stackMainLabel: 'Main',
    certKicker: 'Credentials',
    certTitle: 'Certificate',
    certViewLabel: '자격증 보기 방식',
    certViewMini: '카드로 보기',
    certViewList: '목록으로 보기',
    courseKicker: 'Courses',
    courseTitle: 'Completion',
    courseEmpty: '아직 등록된 수료 내역이 없습니다.',
    blogKicker: 'Section',
    blogTitle: 'Post',
    blogDescription:
      '공부한 내용과 개발하면서 겪은 일, 프로젝트 회고를 기록합니다.',
    blogLinkLabel: '블로그',
    studyLogLinkLabel: '기술 블로그',
    certIssueNumber: '자격번호',
    certIssueDate: '취득일',
    highlights: [
      'Section',
      'Portfolio',
      '직접 만든 프로젝트와 작업 내용을 정리했습니다.',
      'GitHub에서 보기',
      'Section',
      'Post',
      '공부한 내용과 개발하면서 겪은 일, 프로젝트 회고를 기록합니다.',
      '블로그',
    ],
    contactKicker: 'Get In Touch',
    contactTitle: 'Contact',
    contactLinksLabel: '연락처 링크',
    contactLinkLabels: {
      email: '이메일',
      github: 'GitHub',
      linkedin: 'LinkedIn',
      baekjoon: 'Baekjoon Online Judge',
    },
    contactOpenLabel: '문의하기',
    contactCloseLabel: '문의 창 닫기',
    contactFormTitle: '문의하기',
    contactFormDescription: '궁금한 점이 있다면 편하게 연락해 주세요.',
    contactEmailLabel: '이메일',
    contactEmailPlaceholder: '답변을 받을 이메일 주소',
    contactSubjectLabel: '제목',
    contactSubjectPlaceholder: '제목을 입력해 주세요',
    contactMessageLabel: '내용',
    contactMessagePlaceholder: '문의 내용을 적어 주세요',
    contactSubmitLabel: '메일 앱 열기',
    contactFormStatus:
      '메일 앱에서 내용을 확인한 뒤 보내 주세요.',
    contactMailSenderLabel: '답변을 받을 이메일',
    footer: 'Made by Sangsun Kim',
  },
  en: {
    metaDescription:
      'GitHub Pages portfolio homepage for Software Engineer Sangsun Kim',
    brand: 'STARRYKSS',
    nav: [
      'About',
      'Experience',
      'Tech Stack',
      'Certificate',
      'Completion',
      'Contact',
    ],
    heroText: 'Hey, There!\nI am SW Engineer\nSangsun Kim',
    heroActions: ['About', 'Portfolio', 'Contact'],
    quickFacts: [
      'Frontend development with React and Next.js',
      'Experience with TypeScript, Tailwind CSS, Python, Azure, Azure AI Foundry, and Codex',
      'Independent static structure ready for GitHub Pages deployment',
    ],
    aboutKicker: 'Profile',
    aboutTitle: 'About Me',
    githubProfileLabel: 'View starrykss on GitHub',
    githubChartLabel: 'GitHub contributions in the last year. Scroll horizontally to explore.',
    githubLoading: 'Loading GitHub activity.',
    githubError: 'Activity could not be loaded. Visit the GitHub profile above to view it.',
    githubSummary: '{count} contributions in the last year',
    githubDay: 'Contributions on {date}: {count}',
    githubLess: 'Less',
    githubMore: 'More',
    profileRole: 'Software Engineer',
    profileName: 'Sangsun Kim · @starrykss',
    profileDescription:
      'I am interested in designing and building products end to end, looking beyond the frontend to the backend and DevOps. I enjoy learning new technologies and care about taking software beyond “it works” by making it faster, more stable, and easier to maintain. I build cross-platform applications that can be genuinely useful in people’s everyday lives.',
    overviewKicker: 'Overview',
    overviewTitle: 'Current Focus',
    overviewItems: [
      [
        'Primary',
        'Cross-platform interface design and implementation across web and mobile',
      ],
      [
        'Tooling',
        'React, Next.js, React Native, Flutter, TypeScript, Tailwind CSS, Azure AI Foundry, and Codex',
      ],
      [
        'Expansion',
        'Expanding into multi-device product work and AI-enabled development workflows',
      ],
    ],
    experienceKicker: 'Timeline',
    experienceTitle: 'Experience & Education',
    experienceSummary: '',
    experienceGroupTitle: 'Experience',
    educationGroupTitle: 'Education',
    timeline: [
      [
        '2025.02 - Present',
        'PLAYBUNIZ Inc.',
        'Full-stack Development, CTO',
        'React, TailwindCSS, Next.js, TypeScript, Azure, AWS, GCP, React Native, Flutter, MongoDB, PostgreSQL, Figma, Linux',
      ],
      [
        '2024.10 - 2024.10',
        'APST Corporation',
        'Full-stack Development Internship',
        'React, FastAPI, Python, Redis, PostgreSQL',
      ],
      [
        '2024.02 - 2024.04',
        'Torooc Corporation',
        'Frontend Development Internship',
        'React, Styled Components, MobX, AWS, Python, ROS2',
      ],
      [
        '2023.04 - 2023.09',
        'Microsoft Korea AI School',
        'Trainee',
        'HTML/CSS/JavaScript, Python, PyTorch, OpenCV, Flask, PyQt, Azure, Git',
      ],
      [
        '2016.03 - 2023.08',
        'Catholic University of Korea',
        "Bachelor's Degree",
        'Major in Computer Science and Information Engineering, Double Major in Media Engineering',
      ],
    ],
    stackKicker: 'Capabilities',
    stackTitle: 'Tech Stack',
    stackMainLabel: 'Main',
    certKicker: 'Credentials',
    certTitle: 'Certificate',
    certViewLabel: 'Certificate view',
    certViewMini: 'Mini cards',
    certViewList: 'List view',
    courseKicker: 'Courses',
    courseTitle: 'Completion',
    courseEmpty: 'This is a ready section for completed courses such as Udemy.',
    blogKicker: 'Section',
    blogTitle: 'Post',
    blogDescription:
      'A space where I organize new learnings, study notes, and retrospectives from development.',
    blogLinkLabel: 'Blog',
    studyLogLinkLabel: 'Tech Blog',
    certIssueNumber: 'Issue Number',
    certIssueDate: 'Issue Date',
    highlights: [
      'Section',
      'Portfolio',
      'A space where I organize projects I have built while developing.',
      'Open Repository',
      'Section',
      'Post',
      'A space where I organize new learnings, study notes, and retrospectives from development.',
      'Blog',
    ],
    contactKicker: 'Get In Touch',
    contactTitle: 'Contact',
    contactLinksLabel: 'Contact links',
    contactLinkLabels: {
      email: 'Email',
      github: 'GitHub',
      linkedin: 'LinkedIn',
      baekjoon: 'Baekjoon Online Judge',
    },
    contactFormTitle: 'Contact',
    contactOpenLabel: 'Get in touch',
    contactCloseLabel: 'Close contact form',
    contactFormDescription: 'Feel free to reach out anytime!',
    contactEmailLabel: 'Email',
    contactEmailPlaceholder: 'Your reply email',
    contactSubjectLabel: 'Subject',
    contactSubjectPlaceholder: 'What would you like to discuss?',
    contactMessageLabel: 'Message',
    contactMessagePlaceholder: 'Write your message',
    contactSubmitLabel: 'Send',
    contactFormStatus:
      'Your mail app has been opened. Review the message and send it when ready.',
    contactMailSenderLabel: 'Reply email',
    footer: 'Made by Sangsun Kim',
  },
  de: {
    metaDescription:
      'GitHub-Pages-Portfolioseite für Software Engineer Sangsun Kim',
    brand: 'STARRYKSS',
    nav: [
      'About',
      'Experience',
      'Tech Stack',
      'Certificate',
      'Completion',
      'Contact',
    ],
    heroText: 'Hallo!\nIch bin SW Engineer\nSangsun Kim',
    heroActions: ['About', 'Portfolio', 'Kontakt'],
    quickFacts: [
      'Frontend-Entwicklung mit React und Next.js',
      'Erfahrung mit TypeScript, Tailwind CSS, Python, Azure, Azure AI Foundry und Codex',
      'Unabhängige statische Struktur für GitHub Pages',
    ],
    aboutKicker: 'Profile',
    aboutTitle: 'About Me',
    githubProfileLabel: 'starrykss auf GitHub ansehen',
    githubChartLabel: 'GitHub-Beiträge im letzten Jahr. Horizontal scrollen, um mehr zu sehen.',
    githubLoading: 'GitHub-Aktivitäten werden geladen.',
    githubError: 'Aktivitäten konnten nicht geladen werden. Bitte das GitHub-Profil oben öffnen.',
    githubSummary: '{count} Beiträge im letzten Jahr',
    githubDay: 'Beiträge am {date}: {count}',
    githubLess: 'Weniger',
    githubMore: 'Mehr',
    profileRole: 'Software Engineer',
    profileName: 'Sangsun Kim · @starrykss',
    profileDescription:
      'Ich interessiere mich dafür, Produkte ganzheitlich zu konzipieren und zu entwickeln – vom Frontend über das Backend bis hin zu DevOps. Ich lerne gern neue Technologien und gebe mich nicht damit zufrieden, dass Software nur funktioniert, sondern achte auch auf Performance, Stabilität und Wartbarkeit. Dabei entwickle ich plattformübergreifende Anwendungen, die Menschen im Alltag wirklich weiterhelfen.',
    overviewKicker: 'Overview',
    overviewTitle: 'Current Focus',
    overviewItems: [
      [
        'Primary',
        'Plattformübergreifendes Interface-Design und Umsetzung für Web und Mobile',
      ],
      [
        'Tooling',
        'React, Next.js, React Native, Flutter, TypeScript, Tailwind CSS, Azure AI Foundry und Codex',
      ],
      [
        'Expansion',
        'Ausbau von Multi-Device-Produkten und AI-gestützten Entwicklungsabläufen',
      ],
    ],
    experienceKicker: 'Timeline',
    experienceTitle: 'Experience & Education',
    experienceSummary: '',
    experienceGroupTitle: 'Berufserfahrung',
    educationGroupTitle: 'Ausbildung',
    timeline: [
      [
        '2025.02 - Present',
        'PLAYBUNIZ Inc.',
        'Full-stack-Entwicklung, CTO',
        'React, TailwindCSS, Next.js, TypeScript, Azure, AWS, GCP, React Native, Flutter, MongoDB, PostgreSQL, Figma, Linux',
      ],
      [
        '2024.10 - 2024.10',
        'APST Corporation',
        'Full-stack Development Internship',
        'React, FastAPI, Python, Redis, PostgreSQL',
      ],
      [
        '2024.02 - 2024.04',
        'Torooc Corporation',
        'Frontend Development Internship',
        'React, Styled Components, MobX, AWS, Python, ROS2',
      ],
      [
        '2023.04 - 2023.09',
        'Microsoft Korea AI School',
        'Trainee',
        'HTML/CSS/JavaScript, Python, PyTorch, OpenCV, Flask, PyQt, Azure, Git',
      ],
      [
        '2016.03 - 2023.08',
        'Catholic University of Korea',
        'Bachelorabschluss',
        'Hauptfach Informatik, Doppelstudium Medieningenieurwesen',
      ],
    ],
    stackKicker: 'Capabilities',
    stackTitle: 'Tech Stack',
    stackMainLabel: 'Main',
    certKicker: 'Credentials',
    certTitle: 'Certificate',
    certViewLabel: 'Zertifikatsansicht',
    certViewMini: 'Minikarten',
    certViewList: 'Listenansicht',
    courseKicker: 'Courses',
    courseTitle: 'Completion',
    courseEmpty:
      'Dies ist ein vorbereiteter Bereich für abgeschlossene Kurse wie Udemy.',
    blogKicker: 'Section',
    blogTitle: 'Post',
    blogDescription:
      'Ein Bereich, in dem ich neue Erkenntnisse, Lernnotizen und Retrospektiven aus der Entwicklung sammle.',
    blogLinkLabel: 'Blog',
    studyLogLinkLabel: 'Technikblog',
    certIssueNumber: 'Nummer',
    certIssueDate: 'Datum',
    highlights: [
      'Section',
      'Portfolio',
      'Ein Bereich, in dem ich Projekte sammle, die ich beim Entwickeln gebaut habe.',
      'Repository öffnen',
      'Section',
      'Post',
      'Ein Bereich, in dem ich neue Erkenntnisse, Lernnotizen und Retrospektiven aus der Entwicklung sammle.',
      'Blog',
    ],
    contactKicker: 'Get In Touch',
    contactTitle: 'Contact',
    contactLinksLabel: 'Kontaktlinks',
    contactLinkLabels: {
      email: 'E-Mail',
      github: 'GitHub',
      linkedin: 'LinkedIn',
      baekjoon: 'Baekjoon Online Judge',
    },
    contactFormTitle: 'Kontakt',
    contactOpenLabel: 'Kontakt aufnehmen',
    contactCloseLabel: 'Kontaktformular schließen',
    contactFormDescription: 'Melde dich jederzeit gerne!',
    contactEmailLabel: 'E-Mail',
    contactEmailPlaceholder: 'Deine E-Mail für die Antwort',
    contactSubjectLabel: 'Betreff',
    contactSubjectPlaceholder: 'Worum geht es?',
    contactMessageLabel: 'Nachricht',
    contactMessagePlaceholder: 'Schreibe deine Nachricht',
    contactSubmitLabel: 'Senden',
    contactFormStatus:
      'Die Mail-App wurde geöffnet. Bitte prüfe die Nachricht und sende sie ab.',
    contactMailSenderLabel: 'Antwort-E-Mail',
    footer: 'Made by Sangsun Kim',
  },
};
